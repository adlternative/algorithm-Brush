...
`lc 11`
看上去好像是接水的单调栈，实际上是左右俩指针向内收缩，的确难想到。
...
### 前缀和
`leetcode 523`
区间整除。算两个前缀和余数是否相同就好了。

`leetcode 230`
区间差 k。遍历前缀和的时候顺便去看 map 中有没有和自己相差 k 的前缀和。
...
`lc 287`
1. 找重复数字。注意 `2^remain` 是 `1 << remain` 而不是 `remain << 1`。
