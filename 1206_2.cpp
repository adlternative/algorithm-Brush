...
template <typename Type> class SkipList {
public:
  struct Node {
    Type value;                             //存储的值
    uint32_t level;                         //跨越几层
    ...
    Node(Type vl, uint32_t le) : value(vl), level(le), next(new Node[le]) {}
...
public:
  ...
  void Delete(Type value) {
    ...
    /*         for(int i = levelcount - 1; i >= 0; --i){
                while(Temp.next != nullptr && Temp.next[i].next != nullptr &&
       Temp.next[i].value < value){ Temp = Temp.next[i];
                }
                update[i] = Temp;
            } */
    ...
    if (Temp.next[0].next != nullptr &&
        Temp.next[0].value == value) { //重载等于号
      --size;
      for (int i = levelcount - 1; i >= 0; --i) {
        if (update[i].next[i].next != nullptr &&
